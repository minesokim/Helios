...
const ItemCard = forwardRef<HTMLDivElement, ItemCardProps>(
  ({ image, title, delay, isVisible, index, totalCount, onClick, isSelected }, ref) => {
    ...
    return (
      ...
      >
        <div className={cn(
          "w-full h-full rounded-lg overflow-hidden shadow-xl bg-slate-800 border border-white/5 relative",
          "transition-all duration-500 ease-[cubic-bezier(0.16,1,0.3,1)]",
          "group-hover/card:-translate-y-6 group-hover/card:shadow-2xl group-hover/card:shadow-violet-500/40 group-hover/card:ring-2 group-hover/card:ring-violet-500 group-hover/card:scale-125"
        )}>
          {image ? (
            <img
              src={image}
            ...
            <div className="w-full h-full bg-gradient-to-br from-slate-700 to-slate-800 flex items-center justify-center">
              <span className="text-2xl">{title.charAt(0).toUpperCase()}</span>
            </div>
          )}
  ...
  return (
    ...
    >
      ...
      <button
        ...
        className={cn(
          "absolute top-6 right-6 z-50 w-12 h-12 flex items-center justify-center rounded-full bg-slate-800/30 backdrop-blur-xl border border-white/10 shadow-2xl text-white hover:bg-slate-700 transition-all duration-300",
        )}
        style={{
      ...
      <button
        ...
        className={cn(
          "absolute left-4 md:left-10 z-50 w-14 h-14 flex items-center justify-center rounded-full bg-slate-800/30 backdrop-blur-xl border border-white/10 text-white hover:scale-110 active:scale-95 transition-all duration-300 disabled:opacity-0 disabled:pointer-events-none shadow-2xl",
        )}
        style={{
      ...
      <button
        ...
        className={cn(
          "absolute right-4 md:right-10 z-50 w-14 h-14 flex items-center justify-center rounded-full bg-slate-800/30 backdrop-blur-xl border border-white/10 text-white hover:scale-110 active:scale-95 transition-all duration-300 disabled:opacity-0 disabled:pointer-events-none shadow-2xl",
        )}
        style={{
      ...
      >
        <div className={cn("relative overflow-hidden rounded-[inherit] bg-slate-900 border border-white/10 shadow-[0_35px_60px_-15px_rgba(0,0,0,0.5)]")}>
                ...
                <div key={item.id} className="min-w-full h-full relative">
                  {item.image ? (
                    ...
                    <div className="w-full h-full bg-gradient-to-br from-slate-700 to-slate-900 flex items-center justify-center">
                      <span className="text-6xl font-bold text-slate-500">{item.title.charAt(0).toUpperCase()}</span>
                    </div>
                  )}
          ...
          <div
            className={cn("px-8 py-7 bg-slate-900 border-t border-white/5")}
            style={{
              opacity: animationPhase === "complete" && !isClosing ? 1 : 0,
          ...
          >
            <div className="flex items-center justify-between gap-6">
              <div className="flex-1 min-w-0">
                <h3 className="text-2xl font-bold text-white tracking-tight truncate">{currentItem?.title}</h3>
                <div className="flex items-center gap-4 mt-2">
                  <div className="flex items-center gap-1.5 px-2.5 py-1 bg-slate-800 rounded-full border border-white/5">
                    {items.map((_, idx) => (
                      <button
                        key={idx}
                        onClick={() => handleDotClick(idx)}
                        className={cn("w-1.5 h-1.5 rounded-full transition-all duration-500", idx === internalIndex ? "bg-white scale-150" : "bg-slate-600 hover:bg-slate-400")}
                      />
                    ))}
                  </div>
                  <p className="text-xs font-bold uppercase tracking-widest text-slate-500">{internalIndex + 1} / {totalItems}</p>
                </div>
              </div>
              {(currentItem?.href || onItemClick) && (
                <button
                  onClick={() => {
                    if (onItemClick) {
                  ...
                  className={cn("flex items-center gap-2 px-6 py-3 text-sm font-bold uppercase tracking-widest text-white bg-violet-600 hover:bg-violet-500 rounded-xl shadow-lg shadow-violet-600/20 transition-all duration-300 hover:scale-105 active:scale-95")}
                >
                  <span>Open</span>
...
// --- Main Animated Folder Component ---
...
export const AnimatedFolder: React.FC<AnimatedFolderProps> = ({
  ...
  const handleNavigate = (newIndex: number) => { setSelectedIndex(newIndex); setHiddenCardId(items[newIndex]?.id || null); };
  ...
  return (
    <>
      <div
        className={cn(
          "relative flex flex-col items-center justify-center p-8 rounded-2xl cursor-pointer bg-slate-900/50 border border-slate-700/50 transition-all duration-700 ease-[cubic-bezier(0.16,1,0.3,1)] hover:shadow-2xl hover:shadow-violet-500/10 hover:border-violet-500/30 group",
          className
        )}
        style={{
          minWidth: "280px",
      ...
      >
        <div
          ...
          style={{
            background: gradient
              ? `radial-gradient(circle at 50% 70%, ${gradient.match(/#[a-fA-F0-9]{3,6}/)?.[0] || '#8b5cf6'} 0%, transparent 70%)`
              : "radial-gradient(circle at 50% 70%, #8b5cf6 0%, transparent 70%)",
        ...
        <div className="text-center">
          <h3 className="text-lg font-bold text-white mt-4 transition-all duration-500" style={{ transform: isHovered ? "translateY(2px)" : "translateY(0)", letterSpacing: isHovered ? "-0.01em" : "0" }}>
            {title}
          </h3>
