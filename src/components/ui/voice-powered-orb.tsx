  ...
  useEffect(() => {
    ...
    const initWebGL = () => {
      ...
      try {
        ...
        // Initial size
        rendererInstance.setSize(width * (window.devicePixelRatio || 1), height * (window.devicePixelRatio || 1));
        glContext.canvas.style.width = width + "px";
        glContext.canvas.style.height = height + "px";
    ...
    // Try to init after a brief delay to ensure DOM is ready
    const initTimeout = setTimeout(() => {
      initWebGL();
    }, 50);

    // Initialize microphone if voice control enabled
